...
export default function VendorHome() {
  ...
  const { vendorRevenue, isLoading } = useAppSelector(
    (state) => state.vendorRevenue
  );
  ...
  const getOrders = vendorOrders.reduce((sum, vendor) => sum + (vendor.orders || 0), 0);
  const getEarnings = vendorEarning.reduce((sum, vendor) => sum + (vendor.earnings || 0), 0);
  const getRevenue = vendorRevenue.reduce((sum, vendor) => sum + (vendor.revenue || 0), 0);
