...
const EditVendor = ({ handleClose }: any) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { vendor: logedInVendor }: any = useAppSelector((state) => state.vendorAuth);
    ...
    <div className="vendorEditModal">
      <form onSubmit={handleSubmit} className="vendorEditForm">
        ...
        <div id="profile" className="flex flex-row space-x-4">
          <button type="submit" className="bg text-white rounded">
            {loading ? "Loading..." : "Save"}
          </button>
          <button className="bg text-white rounded" onClick={handleCancel}>
            Cancel
          </button>
        </div>
      </form>
